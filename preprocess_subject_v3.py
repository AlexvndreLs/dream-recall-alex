...
- config_v3.py : SFREQ_PREPROC=1000, N_SAMPLES=30000, WINDOW=1000, DECIMATE=False
    (réplication thèse Arthur — voir DECIMATE dans config_v3.py pour revenir à 250Hz)
  - HP final = 0.1Hz -> psd_delta comparable au pipeline Arthur sur ce point
  - Référence nez conservée (apply_average_reference est un no-op, cf fonction plus bas)
    -> identique à Arthur, contrairement à ce que disait ce commentaire avant
...
def run_ica(
    ...
    # détection composantes oculaires via EOG_L, EOG_R ET une voie horizontale
    # dérivée EOG_horiz = EOG_L - EOG_R. L'EOG horizontal (saccades latérales,
    # nombreuses en REM) forme un dipôle gauche-droite : EOG_L et EOG_R y sont
    # en OPPOSITION, donc chaque voie seule corrèle faiblement et passe sous le
    # seuil. Leur différence capture ce dipôle. La voie bipolaire est créée sur
    # une copie locale (détection seulement) -> ne reste pas dans les données.
    # threshold=2.5 (défaut MNE 3.0) : plus sensible, attrape l'oculaire modéré.
